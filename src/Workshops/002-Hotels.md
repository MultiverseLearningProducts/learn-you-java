...
📌 Remember the SOLID principles mentioned earlier? _Minimal interfaces_ in the _SOLID_ principles is called **Interface segregation principle** it is the **I** in S.O.L.**I**.D. principles.

> A client should never be forced to implement an interface that it doesn’t use, or clients shouldn’t be forced to depend on methods they do not use.

This principles encourages us to expose only what is needed. We need these interfaces of `checkIn` and `isEmpty` so let's update our classes to have these interfaces and make sure there is somewhere cosy for our guests to sleep!
